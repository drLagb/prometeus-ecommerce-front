...
function IconAnimation({className}:any){
    ...
    if(!estado){
        return(
        <IoHeartOutline className={"text-black opacity-80 " + 
    ...
    return(
    <IoHeartSharp  className={"text-rojoColor opacity-80 " + 
        "text-mv-icon-m md:text-pc-icon-m " +
        "md:hover:opacity-100 md:transition-all " +
        "absolute top-0 bottom-0 right-0 left-0  m-auto z-10 " +
        "animate-in spin-in duration-400"
        + " " + className}
        onClick={handleClick}
        title="eliminar"
        role="button"/>
...
export default function HeartIcon({className, sonsClassNames}:Props){
